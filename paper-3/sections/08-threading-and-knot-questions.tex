...
The sextic Lyashko--Looijenga laboratory shows that this horizontal mechanism
is not confined to two-strand torus links.  For the single integral pencil
$P_0(x)-t=x^6-x-t$, the normalized discriminant
$Q_0(t)=t^5+5^5/6^6$ is the exact schedule of five singular fibers.  At the
regular slice $t=0$, the five radial vanishing arcs form a spanning star on the
six roots, and their half-twists generate $B_6$.  Hence loops in one fixed
five-punctured parameter plane realize all six-braids and, after the faithful
Birman--Hilden lift with boundary and subsequent capping, all genus-two mapping
classes.  The integral Gauss--Manin image is $\operatorname{Sp}_4(\Z)$.

The same example makes a second kind of forgetting finite.  The
Lyashko--Looijenga map $P\mapsto Q_P$ has $1296$ normalized lifts over $Q_0$,
or $216$ orbits after the residual $\mu_6$ source rotations.  It preserves the
critical-value schedule while forgetting the six-sheet gluing realization.
This forgetting is now proved to reach genuine complex geometry: an explicit
second LL sheet $P_1$, outside the source-rotation orbit of $P_0$, has the same
$Q_0$ but gives at $t=1$ a genus-two curve with a different absolute
Igusa--Clebsch ratio.  Thus the slice moduli map does not factor through the
LL event polynomial even on this single finite fiber.
The same observable admits two exact readings of the forgotten difference.
Across the degree-$216$ quotient LL fiber, its norm polynomial, traces, and
higher symmetric functions descend to the event-polynomial base; averaging
canonically separates constants from a zero-sum summand, whose Hermitian norm is
a monodromy-invariant variance with a strict positive lower bound at $Q_0$.
The value returns around closed LL loops but need not remain constant along an
arbitrary open path of event polynomials.  Along the two explicit $t$-pencils,
the invariant ratio is $((t-\beta)/t)^5$, giving the balanced divisor charge
$5[\beta]-5[0]$ at two regular fibers.  This logarithmic charge is not a finite
Dirichlet energy; a Hodge- or Siegel-metric refinement remains open.
At $t=1$, the branch polynomial $x^6-x-1$ has Galois group $S_6$, while the
curve's Jacobian has geometric endomorphism ring $\Z$; thus the maximal
monodromy is not caused by accidental
complex multiplication.  These classical results and the explicit twin theorem now
inhabit one carrier--thread--spectral construction.  What remains open is the
full census of all $216$ source-rotation orbits, comparison of arithmetic and
period data, and the stronger test after fixing marked homological monodromy.
