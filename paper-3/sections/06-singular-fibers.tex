...
There is nevertheless a precise relation to
Theorem~\ref{thm:branched-pullback-zero-germ}.  If the same local map is regarded
spatially as $\Phi(z)=z^2$ and one takes the real assignment
$\im\Phi$, its zero germ is four-pronged and its pulled-back AEG metric has cone
angle $4\pi$.  In the present subsection, by contrast, $w^2=\tau$ is an incidence
over a parameter plane and the output is root monodromy.  The same holomorphic
branch supplies two constructions only after the roles of source, target, and
parameter have been declared.
