...
Paper~0 associates to continuous affine evaluation a regular arithmetic expression
space
\[
  (M,g,a;\mu,\lambda),
  \qquad
...
real-valued assignment function \cite{Yuan2026AEGPaperZero}.  The right-hand
...
The family $z^2-t^m$ below makes this distinction explicit.  The fixed sextic
pencil $x^6-x-t$ then makes the relation nonabelian: its five critical events
generate every six-braid and, after hyperelliptic lifting, every genus-two
mapping class.  Two explicit normalized sextics over its one LL event
polynomial give nonisomorphic genus-two curves at the common slice $t=1$,
so the event schedule strictly forgets complex moduli.  These are supplied
relations rather than general outputs of
expression history, but their arithmetic factorization, analytic period,
carrier topology, and braid data can be compared without changing the
underlying object.
...
Sextic LL laboratory & For $P_0=x^6-x$, the event polynomial is
$Q_0(t)=t^5+5^5/6^6$; its five star half-twists generate $B_6$, so the same
pencil realizes every genus-two mapping class and has full
$\operatorname{Sp}_4(\Z)$ Gauss--Manin image.  The LL fiber has $1296$
coordinate-normalized lifts, or $216$ residual source-rotation orbits; an
explicit second orbit has the same event polynomial but a different genus-two
Igusa moduli reading at $t=1$.  The resulting sheet observable has spectral
trace--norm descent and positive monodromy-invariant forgetting variance; the
two slice pencils have balanced divisor charge $5[\beta]-5[0]$. &
Proved \\
...
Section~\ref{sec:sextic-ll-laboratory} studies the fixed pencil
$x^6-x-t$: its LL event polynomial, full six-braid and genus-two monodromy,
nested discriminants, arithmetic fiber, and the explicit LL--Igusa
nonfactorization theorem, together with its finite-sheet spectral descent,
forgetting variance, and moving-slice divisor charge.  The full $216$-orbit
census and its arithmetic, period, marked-monodromy, and Hodge/Siegel-energy
