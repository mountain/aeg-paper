...
independently; for the basic model the contact model is proved to be a Reeb
quotient of the hyperbolic model, with the horizontal fields descending to the
canonical frame.  We prove that the arithmetic tower generates an
infinite-dimensional Lie algebra, so that no finite-dimensional Lie-group action
carries all three ranks, and that no observable of the commutativized charges can
detect tearing; both boundaries are stated as boundaries, and the faithful affine
matrix representation is recorded alongside them.  The paper closes with the
resulting methodological position, a register of computed witnesses for punctured
spaces, and the open problems.
\end{abstract}
...
\input{sections/08-acs-tearing.tex}
