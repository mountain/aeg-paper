...
The theorem answers the question the review posed, and it is worth noting what
it does *not* do.  It does not make the puncture set canonical: $S=\{0\}$ is
still stipulated, although here it is forced by the data in the weaker sense that
the assignment of the model is critical there.  It does not attach the
configuration to the arithmetic motions rather than to the assignment, and it
does not treat more than one puncture.  Finally it makes the interface with
...
\begin{enumerate}
  ...
  way, and in particular can a $k$-punctured model be produced without
  stipulating $S$?  And does the construction survive if the assignment is
  required to come from the arithmetic motions rather than being prescribed as a
  polynomial?  A negative answer to the second question would mean that the
  configuration is realizable as a zero locus but not as an expression.
  ...
  first unfinished item of the exploration register's own programme; the model of
  Theorem~\ref{thm:p0-four-circle-model} forces its puncture in the weaker sense
  that the assignment is critical there, and whether that suffices is the
  question.
