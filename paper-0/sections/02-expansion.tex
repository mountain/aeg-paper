...
Two different expansions may induce the same arithmetic operator.  The law
responsible for this is distributivity, and it is worth isolating because every
later chapter compares expansions that share an operator or a value.
...
induced operator $z\mapsto kz+kp$ on $K$, but different constant data:
$(p,k)$ in the first word and $(kp,k)$ in the second.
\end{definition}
...
\end{equation}
When $k\ne1$ and $p\ne0$ the two words are distinct marked expansions: their
chronological constant words $(p,k)$ and $(kp,k)$ differ.
\end{proposition}

\begin{proof}
Both sides send $z$ to $kz+kp$, which is the distributive law in $K$.  If the
constant words agreed, then $p=kp$, that is $(k-1)p=0$; since $K$ is a field
this forces $k=1$ or $p=0$.
\end{proof}
