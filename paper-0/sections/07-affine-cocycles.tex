% Moved from Paper I Section 4 (governance/00b, migration map); mathematical content
% unchanged apart from cross-references and framing sentences.
We now restrict attention to the affine motions of Section~\ref{sec:p0-aes}.  Every
...
point.  These two coordinates are the discrete data whose commutativized shadow
is constructed in Section~\ref{sec:p0-acs-tearing}.
...
comparison with path inverse, or a mirror invariant.
Section~\ref{sec:p0-acs-tearing} places compatible two-history defects in the
global torsion framework.
...
continuous arithmetic motion of Section~\ref{sec:p0-aes} is obtained by
differentiating the same semidirect-product action.  Supplementary expansions
and convention checks appear in Appendix~\ref{app:p0-affine-cocycles}.
