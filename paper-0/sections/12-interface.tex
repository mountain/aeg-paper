...
Two chapters of this paper are deliberately *not* offered as stable interfaces
to the later papers.  Section~\ref{sec:p0-beyond-linear} states two proved
boundaries --- the boundary of finite-dimensional Lie-group closure and the
boundary of observables of the charge endpoint --- together with a vocabulary
imported from a companion programme; the boundaries are theorems, the vocabulary
is a research stance, and the later papers depend on neither.  Similarly, the
*witness register* of Section~\ref{subsec:p0-holed-witnesses} and the two
registered problems of Subsection~\ref{subsec:p0-holed-open} are research
reports.  That chapter is not, however, without proved content: the definition of
a punctured space, the non-vanishing of the assignment gradient in a regular AES,
and the explicit punctured model realizing the four-circle configuration
(Theorem~\ref{thm:p0-four-circle-model}) are theorems, and the last of them may be
imported wherever a concrete punctured instance is needed in place of the disc
model of Paper~I.
...
history group and charge descent & what the ACS is, and what the motions keep \\
...
the Reeb-quotient identification & the two models are one object \\
punctured models and their zero loci & a verified once-punctured instance \\
tearing and the hole witnesses & research report; not yet an interface \\
\bottomrule
\end{tabular}
\end{center}
