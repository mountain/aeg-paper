...
two translation coordinates.  This section constructs the *commutativization* of
that non-commutative motion.

Commutativization here means the following.  Keep the total additive charge and
the total logarithmic multiplicative charge of a history, and forget the order
in which they were accumulated; the charges of an additive step and of a scaling
step then commute by construction, and the resulting object is a plane with two
coordinates.  We call it the \emph{accumulative commutative space}, ACS.  The
construction has two distinct uses, and they must not be conflated.  For a
positive add--scale history it records the two total charges; and it supplies a
weighted integral along the charge path whose value restores the translation
part that the charge endpoint alone forgets.  The ACS is therefore a *shadow* of
the presented history: it is the commutative quotient object attached to a
non-commutative one, not another presentation of the full arithmetic syntax.

The second use is the geometric one, and it is the reason for the name of this
chapter.  Two histories with the same charge endpoint have charge paths that
...
single motion, and the geometry in which it becomes an area is precisely the
commutativization of that space.
...
Let $\gamma$ and $\delta$ be two arithmetic histories of a regular AES that are
...
hence agree on the multiplicative content of the motion, yet their additive
content differs, and the difference is exactly the weighted ACS filling between
their charge paths.
\end{definition}
...
\begin{enumerate}
  ...
  \item \emph{It is measured, not merely detected.}  The value
  $\tau(\gamma,\delta)$ is an exact weighted area
  \eqref{eq:torsion-area-integral}, and for the elementary rectangle it is
  ...
  shows that this coefficient is the curvature of a contact structure, hence
