...
will begin where the present horizontal connection stops.  Its program is to
choose and analyze additional metric and analytic data: a horizontal metric,
...
form alone, and no result of that prospective function theory is used in the
proofs of Paper~I.  The $G_4$ sublanguage supplies an additional, explicitly
arithmetic test case: an automorphic or equivariant function used there must be
...
will study the boundary of the regular-zero theorem.  Its intended objects
include singular and multiple zeros, parameter discriminants, regular and
singular zero surfaces, and tube constructions.  Possible braid, monodromy,
and knot structures require separate invariance and normalization arguments.
The Hecke interface proved here offers a source-driven alternative to imposing
a four-valent cellulation by hand: Paper~III may ask whether an analytic real
locus associated with $G_4$ pulls back to a singular zero graph supported by a
Hecke cellulation.  Neither that analytic realization nor the identification
of its local singularities is a result of Paper~I.  The complete regular
...
Computational Complexity} will return to the full projective process--result
hierarchy.  Its proposed scope includes bivaluation, quotient and condensation
...
proved in this paper supplies an entry point, but it does not establish any
general complexity theorem.  Those claims remain open until precise models of
representation, input size, and computation are fixed.
...
\begin{enumerate}
    ...
    \item Construct a functorial analytic realization of the $G_4$ history--operator--coset
    interface and determine which Hecke cellulation, if any, is recovered as a
    real or zero locus.
