...
2. This `AGENTS.md`.
3. Authoritative files under `governance/`.
4. The current main paper source.
5. Research notes under `notes/`, `gpt/`, `../archive/plans/`, or other archival directories.

Files under `governance/` should explicitly identify whether they are authoritative specifications, status records, audits, or archival discussions.
...
Before performing any restructuring task, read the relevant files under `governance/`.
...
* `governance/06-editorial-rules.md`
* `governance/07-acceptance-checklist.md`
...
The files under `governance/discussions/` summarize research routes but are subordinate to the authoritative scope and mathematical-status files.

The files under `governance/archive/` are archival and must not be treated as current decisions.
