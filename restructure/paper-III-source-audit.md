...
1. restructure/AGENTS.md and the explicit task;
2. restructure/00-authoritative-scope.md through
...
5. working discussions under restructure/discussions/;
...
- notes/, knots/, misc/, images/, and archive/;
- restructure/04-current-to-target-map.md,
  restructure/05-mathematical-status.md,
  restructure/08-open-questions.md, and both Paper III working discussions;
...
- the old version of notes/note_08.tex titled “Second Kind AEG-S2” is the same
...
| restructure/00-authoritative-scope.md | lines 392–508 and 758–852 | Model audit, family-zero-set limits, and the knot-theorem gate | AUTHORITATIVE constraint |
| restructure/01-paper-series-architecture.md | lines 1079–1123 and 1512–1527 | Canonical notation and the eight-part Paper III order | AUTHORITATIVE structure |
| restructure/04-current-to-target-map.md | lines 1509–1653 | Reprove multi-zero formulas; distinguish seven zero/tube/knot levels; hold knot claims | AUTHORITATIVE migration rule |
| restructure/05-mathematical-status.md | lines 1410–1564 and 2210–2398 | Proper-submersion boundary and open discriminant, monodromy, threading, and invariant questions | AUTHORITATIVE except for the unsupported E_k and E_{\log} provenance labels corrected below |
| restructure/08-open-questions.md | OQ-012, OQ-019–021, OQ-037–046 | Reject numerical model naming; require properness, embedded moving points, collision avoidance, ambient isotopy, and Markov descent | AUTHORITATIVE open-problem boundary |
...
| 2025-05-16 | commit bf8959d365194c0361b7e35ff9147c524795585c; notes/note_02.tex | Distinguishes a geometric family of AES manifolds from an algebraic evaluation family over evaluation parameters | RETAIN conceptual distinction; reject unsupported dimension and bundle assertions |
| 2026-08 | restructure/discussions/aeg-tube-braid-markov-invariants.md | Separates boundary braids from singular fillings and develops Markov and affine no-go calculations | WORKING DERIVATION; used only where the active manuscript re-proves the calculation |
| 2026-08 | restructure/discussions/three-branch-arithmetic-tubes-and-complexity.md, lines 1250–1296 | Records that no general E_k, E_{\log}, intrinsic thread, braid lift, or Markov-normalized invariant source was located | AUDIT CORROBORATION |
...
| notes/note_02.tex, lines 25–62 | Multiplicative/additive evaluation families; E_alg and E_geom distinction | The distinction between algebraic evaluation and geometric parameter families | Calling the fibers “typically two-dimensional” or the disjoint union a bundle without a topology |
| notes/note_03.tex, lines 23–38 | Proposed figure-eight-exterior model using a lifted fiber and eikonal propagation | A candidate research program | Smooth global propagation, equivariance, descent, cut-locus control, completeness, properness, and total-geodesicity are not proved |
| notes/note_03.tex, lines 40–47 | Golden-ratio calibration | The Alexander root may motivate a multiplier | If the AEG multiplier is t=e^\lambda, the compatible parameter is \(\lambda=\log t\), not \(\lambda=t\) |
| notes/note_04.tex, lines 23–83 | Linear system \(\mu_u/t=\mu_u+\mu_v\), \(\mu_v/t=\mu_u+2\mu_v\), determinant \(t^2-3t+1\) | RETAIN as an explicit affine presentation calculation | It does not give a faithful knot-group geometry |
| notes/note_04.tex, lines 94–117 | Translation generators commute | RETAIN as the stated obstruction to faithfulness | Any earlier embedding language is superseded |
| notes/note_05.tex, lines 25–63 | Lattice generated by (1,0) and (0,\varphi) | The lattice is an ordinary discrete rectangular lattice | “Dense singular torus” is false in \(\mathbb R^2\); density occurs only after a specified one-dimensional projection |
| notes/note_08.tex, lines 23–99 | One isolated zero in the Poincare disc | Superseded by the verified Paper I model | Not a multi-zero construction and not evidence for E_k |
| notes/loop_01.tex, lines 159–281 | Curved-grid intersection formula and assignment \(-x/y=-R/c\) | RETAIN after a sign and domain audit as a visualization calculation | The map w=-1/z is holomorphic and orientation preserving, contrary to line 285 |
| notes/loop_02.tex, lines 85–136 | Six-layer zero taxonomy and a zero signature | RETAIN only as relation-theory vocabulary | Historical subscripts do not canonically count isolated singularities |
| images/13-tube1st.png and misc/13-tube1st.tex | Four-slice tube picture; the TeX file generates only one component of the image | HISTORICAL VISUAL | The picture proves neither smooth incidence nor isotopy |
| images/14-apeirogon.png and misc/14-apeirogon.tex | Apeirogon picture and sparse axis overlay | HISTORICAL VISUAL | No formula links it to an AEG E_{\log} construction |

### 5.2 Affine, Alexander, and knot material

| Historical source | Exact material | Present treatment |
|---|---|---|
| notes/note_11.tex, lines 34–70 | Fox coefficient \(1-y\sim-\lambda\) and the first-order comparison with \(\pm\mu\lambda\) | RETAIN as an analogy, not an isomorphism |
...
| notes/loop_01.tex, lines 98–150 | Explicit grid points and closure when the selected displacement vanishes | RETAIN as a free-word calculation |
| knots/knots_01.tex, lines 8–59 | Figure-eight presentation and the affine word yielding \(x-(t^2-3t+1)\) | RETAIN only with the qualification that the generator assignment descends to the knot group on the relator locus |
| knots/knots_01.tex, lines 66–83 | Claimed Sage/SnapPy checks through eleven crossings | HOLD until scripts, versions, presentations, and an immutable external revision are supplied |
| knots/knots_02.tex | Three-parameter group law and claimed knot-group embeddings | REJECT embedding claims; the stated exponentiation interpretation is not represented by the displayed closed group law |
| knots/knots_03.tex, lines 110–165 | Conditional Alexander factorization and explicit 6_2/6_3 calculations | RETAIN as presentation calculations; the factorization is conditional, not a new invariant |
| knots/results.tex, lines 8–228 | Raw data for 4_1, 6_2, 6_3, and 7_6 under different presentations and cyclic relators | RETAIN primarily as negative evidence that raw torsion depends on presentation, basepoint, sign, and normalization |
| knots/knots_04_zh.tex, lines 162–229 | Polished restatement of the same tables | No independent proof authority; “intrinsic” and “faithful” claims are rejected |
| misc/knot_4_1.tex, lines 20–131 | Reproducible arithmetic-grid drawing for images/knot_4_1.pdf | RETAIN as a figure source after convention review |
...
- restructure/05-mathematical-status.md described E_k as partially proved and
...
| note_13 tube source | INCORRECT SOURCE ATTRIBUTION | notes/note_13.tex belongs to the Paper IV resource/computation program. |
...
2. The distinction in notes/note_02.tex between ambient geometric families and
...
12. The three-parameter constructions in knots/knots_02.tex embed the knot group.
...
restructure/discussions/aeg-tube-braid-markov-invariants.md are a bibliography
seed, not yet a verified bibliography.

## 12. Known provenance-map corrections

1. References in restructure/discussions/aeg-tube-braid-markov-invariants.md that
   assign tube or logarithmic geometry to notes/note_13.tex are incorrect.
...
3. Duplicate README paths in the source table of
   restructure/discussions/three-branch-arithmetic-tubes-and-complexity.md describe
...
5. The current paper-3/ files are an active manuscript closure.  The earlier
   statement in restructure/source-inventory.md that Paper III was only a README
