...
\begin{proof}
    There are two steps in the proof. First, we prove that there exists a metric $g$ on $S$ that makes $a$ satisfy the flow equation~\eqref{eq:flow} in a local coordinate system.
    Then we prove that the local metric can be extended to a global metric on $S$.

    \emph{Part 1: local perspective}

    For an area $U$ of point $p$ on surface $S$, there exists a local isothermal coordinate that satisfies the following equation:
    $$ds^2 =e^{2\rho}(du^2 + dv^2)$$
    where $u$ and $v$ are the coordinates of $U$ and $\rho$ is the function of $u, v$ on $U$.
    The gradient of $a$ in the local isothermal coordinate is given by
    $$\nabla a = \frac{\partial a}{\partial u} du + \frac{\partial a}{\partial v} dv$$
    According to the directional derivative definition, we have
    $$\frac{da_{\phi}}{ds_{\phi}} = ||\nabla a|| \cos \phi$$
    where $||\nabla a||$ is the norm of $\nabla a$ and $\phi$ is the angle between $\nabla a$ and the movement direction.

    Now considering the flow equation in the gradient-contour coodinate, we have the following equation:
    ...
    So can we scale $\rho$ with a linear factor to make $||\nabla a||$ varying to a fixed value of $\sqrt{\mu^2 + a^2 \lambda^2}$?
