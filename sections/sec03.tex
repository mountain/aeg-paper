...
The brachistochrone curve is the curve between two points
that minimizes the time taken by a frictionless bead sliding along a wire.
...
\begin{proof}
    ...
    Thus, we can scale \( e^{2\rho} \) with a linear factor \( \kappa \) to make \( ||\nabla a|| \) match the fixed value of \( \sqrt{\mu^2 + a^2 \lambda^2} \).
    Specifically, we can find a morphing ratio \( \kappa \), and along certain direction, we have:

    \begin{align}
    ds^2 &= e^{2 \rho + \ln \kappa}(du^2 + dv^2) \\
         &= \kappa e^{2 \rho}(du^2 + dv^2)\label{eq:morphing}\\
         &= \kappa ||\nabla a|| \\
