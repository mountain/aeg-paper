...
There are two existence theorem for the flow equation~\eqref{eq:flow}.

\textbf{The first existence theorem}
...
We do not discuss the proof of this theorem here, because it requires further topics in later sections,
the proof is given in the section.

\textbf{The second existence theorem}

\begin{theorem}
    Giving a smooth surface $S$, and a function $a$ on $S$, there exists a metric $g$ on $S$ that makes $a$
...
William P. Thurston had given a famous example\cite{Thurston1994OnPA} of many ways of understanding a mathematical concept.
In this section, we will give a new understanding of functions in which we treat them as flows.
