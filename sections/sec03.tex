...
From equation~\eqref{eq:gradevo2} or equation~\eqref{eq:gradevo3}, we can derive a geometrcal propagation interpretation of the flow equation.
Let's rewrite the equation~\eqref{eq:gradevo2} and equation~\eqref{eq:gradevo3} as

\begin{equation}
    a = \pm \frac{\mu}{\lambda} \sinh(\lambda s \cos \phi + \arcsinh \frac{a_0 \lambda}{\mu})\label{eq:gradevo4}
\end{equation}

If we take $a_0 = 0$, and considering the gradient direction $\phi = 0$, we have

\begin{equation}
    a = \pm \frac{\mu}{\lambda} \sinh(\lambda s)\label{eq:gradevo5}
\end{equation}

Comparing the equation~\eqref{eq:gradevo5} with the circumference of a circle with radius $s$:

\begin{equation}
    C = 2 \pi \frac{1}{sqrt{-k}} \sinh(\sqrt{-k} s) \label{eq:circle}
\end{equation}

This hints at a geometric interpretation of the flow equation, where the assignment $a$ is propagated along the circle with radius $s$.
With a initial assignment $a_0 = 0$, which forms a zero line, and the propagation form a wavefront by circles from every point on the zero line,
then the propagation of $a$ is the propagation of the wavefront.

We will establish a more detailed and rigours geometric interpretation of the flow equation in the section\ref{sec:curvature}.
