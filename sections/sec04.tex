This section introduces the zeroth kind arithmetic expression space ($\mathfrak{E}_0$), providing a geometric framework for analyzing arithmetic expressions. The space is constructed on the upper half-plane with a hyperbolic metric: $ds^2 = \frac{1}{y^2} (dx^2 + dy^2)$, where the assignment function $a = - \frac{x}{y}$ satisfies the flow equation and serves as an eigenfunction of the Laplacian with eigenvalue 2.

Two equivalent examples of $\mathfrak{E}_0$ are presented: the upper half-plane model and a horocycle-based coordinate system, connected through Möbius transformation. The section explores geometric propagation mechanisms, showing how the assignment value propagates like expanding concentric circles in hyperbolic space. It examines grid structures in $\mathfrak{E}_0$, revealing dual grids reflecting the geometric structure of the Baumslag-Solitar group, and demonstrates how arithmetic torsion corresponds precisely to hyperbolic areas enclosed between evaluation paths. The section concludes by introducing tube structures, which extend $\mathfrak{E}_0$ to parameterized families, enabling analysis of how expressions evolve across parameter variations.

At the end of this section, we establish the theoretical framework for the first kind arithmetic expression space $\mathfrak{E}_1$, highlighting its singular properties.

In earlier drafts, the smooth, non-singular arithmetic expression space was denoted by $\mathfrak{E}_1$. In the present work, we adopt a revised indexing: the non-singular case is denoted by $\mathfrak{E}_0$, while $\mathfrak{E}_1$ refers to the arithmetic expression space with a single isolated singularity.

\subsection{Foundational exemplars}\label{subsec:motivexamples}

We present two analytically equivalent examples that belong to the class of spaces designated as the zeroth kind arithmetic expression space $\mathfrak{E}_0$.

\subsubsection{Zeroth kind example 1: Upper Half Plane Model}
...
\subsubsection{Zeroth kind example 2: Horocycle-Based Coordinate System}
...
These two examples, emerging from the same geometric foundation but expressed in different coordinate systems, demonstrate the fundamental properties of the zeroth kind arithmetic expression space.

\subsection{Theoretical framework of $\mathfrak{E}_0$ space}\label{subsec:generalframework}

Building upon the foundational exemplars, we now establish a comprehensive theoretical framework for the zeroth kind arithmetic expression space $\mathfrak{E}_0$.
...
This defines the zeroth kind arithmetic expression space $\mathfrak{E}_0$, characterized by the following theorem:
...
The $\mathfrak{E}_0$ space is distinguished by its intrinsic connection to hyperbolic geometry and the property that the assignment function $a = -x/y$ constitutes an eigenfunction of the Laplacian operator with eigenvalue 2. This space provides a natural geometric framework for analyzing arithmetic expressions, particularly those involving addition and multiplication operations.

\subsection{Geometric propagation mechanisms}\label{subsec:geompropagation}

The flow equation \eqref{eq:flow} in the $\mathfrak{E}_0$ space, $da/ds = \mu \cos \theta + a \lambda \sin \theta$, provides a dynamic interpretation of arithmetic expression evaluation as a propagation process. This perspective extends the propagation method discussed in Section \ref{subsec:propagation-method}.
...
This dynamic picture of equipotential rays sweeping from vertical towards horizontal alignment provides a core geometric interpretation of the propagation mechanism inherent in the flow equation within the $\mathfrak{E}_0$ space.

\subsection{Grid structures, chirality, and their interrelation via conformal mapping}\label{subsec:grids_revised}

A significant geometric characteristic of the zeroth kind arithmetic expression space ($\mathfrak{E}_0$) is the presence of two distinct yet interrelated grid structures. These structures provide a geometric realization for arithmetic operations and reveal a deep connection to the Baumslag--Solitar groups $BS(2,1)$ and $BS(1,2)$. Both grid types are constructed within the upper half-plane model $\mathcal{H} = \{(x,y) \mid y>0\}$, utilizing the assignment function $a = -x/y$ as the underlying scalar field that defines the meaning of arithmetic operations.
...
\caption{Rectilinear grid structure in $\mathfrak{E}_0$, a geometric realization of $BS(2,1)$ (with multiplication factor 2), exhibiting a clockwise (right-handed) operational chirality.}\label{fig:grid1_revised}
\end{figure}
...
\caption{Transformed (curved) grid structure in $\mathfrak{E}_0$ via $w=-1/z$. Through chirality preservation, its operations (effective addition step $-s$, effective multiplication factor $1/2$) correspond to $BS(1,2)$.}\label{fig:grid2_revised}
\end{figure}

\subsubsection{Interrelation and Symmetry}
The rectilinear grid (realizing $BS(2,1)$ with $k=2$) and the transformed curved grid (realizing $BS(1,2)$ with $k=1/2$) are intrinsically linked by the conformal map $w=-1/z$. This geometric transformation, when augmented by the principle of preserving a defined operational chirality, reveals a profound connection between these two distinct Baumslag--Solitar groups. It is not merely a visual transformation of grid lines but a process that systematically reinterprets the arithmetic operations (addition step $s \to -s$, multiplication factor $k \to 1/k$) to maintain a fundamental handedness. This framework elucidates how different algebraic structures can manifest from the same underlying geometric space $\mathfrak{E}_0$ and assignment field $a=-x/y$, connected by conformal symmetry and consistent operational principles.

\subsection{Torsion under scale transformation}\label{subsec:gridsandtorsion}

The addition-multiplication grid introduced in Section~\ref{subsec:meshgrid} has a natural embedding in the arithmetic expression space $\mathfrak{E}_0$. This grid consists of two orthogonal families of curves:
...
We define a \textbf{tube structure $\mathcal{T}$} as the \emph{total space} formed by the family of $\mathfrak{E}_0$ spaces indexed by a continuous parameter $\lambda$ (typically $\lambda > 0$), which can be formally written as the disjoint union:
\begin{equation}
...
\begin{itemize}
    \item \textbf{In the Tube Structure $\mathcal{T}_1$ based on $\mathfrak{E}_0$}: For the $\mathfrak{E}_0$ space ($a=-x/y$) that we have discussed in detail, the zero locus within \textbf{each slice} $\mathfrak{E}_1^{(\lambda)}$ is always the \textbf{same simple} line: the y-axis ($x=0$). Consequently, in the tube structure $\mathcal{T}_1 = \bigsqcup \mathfrak{E}_1^{(\lambda)}$, the overall zero locus is the trivial hyperplane $x=0$.

    \item \textbf{Outlook for Non-Trivial Spaces}: However, as our research suggests, the simplicity of the zero locus in $\mathfrak{E}_0$ might limit its capacity to explain more complex phenomena (like those observed in knot theory examples). Therefore, there is strong motivation to seek and construct \textbf{"non-trivial" arithmetic expression spaces $\mathfrak{E}_{NT}$}, where a single slice $\mathfrak{E}_{NT}^{(\lambda)}$ might possess \textbf{multiple or morphologically more complex zero lines}. In the tube structures $\mathcal{T}_{NT}$ built from such non-trivial spaces, the zero locus itself could evolve with $\lambda$, potentially exhibiting various interesting phenomena, such as:
    \begin{itemize}
...
\subsection{The first kind arithmetic expression space $\mathfrak{E}_1$}\label{subsec:firstkindspace}

We now introduce the \emph{first kind} arithmetic expression space, denoted
$\mathfrak{E}_1$, which represents the simplest instance of an AEG space with
a genuine topological singularity.

In contrast to the zeroth kind space $\mathfrak{E}_0$, which is non-singular and
typically features a line-like zero set (as in the upper half-plane model),
$\mathfrak{E}_1$ is organized around a \emph{single isolated zero point}.
This point acts as a source for arithmetic propagation and introduces the
first non-trivial homological structure into the theory.

\paragraph{Geometric setting.}
The underlying manifold of $\mathfrak{E}_1$ is a hyperbolic plane of constant
Gaussian curvature $K=-\lambda^2$, modeled here by the Poincaré disk
\[
\mathbb{D}^2=\{w\in\mathbb{C}:|w|<1\}.
\]
The center $w=0$ is designated as the unique zero of the assignment function,
$a(0)=0$, and is treated as a singular point of the arithmetic geometry.

Let $s$ denote the hyperbolic distance from $w=0$.
In the Poincaré disk model, if $r_e=|w|$ is the Euclidean radius, then
\[
s(r_e)=\frac{2}{\lambda}\operatorname{arctanh}(r_e),
\qquad \lambda s = 2\operatorname{arctanh}(r_e),
\]
so that $s\to\infty$ as $r_e\to1$.

\paragraph{Assignment function.}
By radial symmetry, the assignment depends only on $s$ and satisfies the
AEG flow equation along the gradient direction,
\[
\frac{da}{ds}=\sqrt{\mu^2+(\lambda a)^2},
\qquad a(0)=0.
\]
The solution is
\[
...
\]
which, expressed in terms of the Euclidean radius $r_e$, becomes
\[
a(r_e)=\frac{\mu}{\lambda}\sinh\!\bigl(2\operatorname{arctanh}(r_e)\bigr)
      =\frac{\mu}{\lambda}\frac{2r_e}{1-r_e^2}.
\]
Thus $a$ vanishes at the singular point $r_e=0$ and diverges as $r_e\to1$.

\paragraph{Characteristic lines.}
The geometry of $\mathfrak{E}_1$ is organized by several canonical families of
curves:
\begin{itemize}
\item \emph{Contour (similarity) lines} $a=\text{const}$ are Euclidean circles
centered at the origin.
\item \emph{Gradient (popularity) lines} are radial geodesics emanating from the
singular point.
\item \emph{Additive and multiplicative directions} are determined by the angle
\[
\alpha(a)=\arctan\!\left(\frac{\mu}{\lambda a}\right)
        =\arctan\!\left(\frac{1-r_e^2}{2r_e}\right),
\]
which depends only on $r_e$.
Near $r_e\to0$, additive directions are tangential and multiplicative directions
are radial; near the boundary $r_e\to1$, this behavior is reversed.
\end{itemize}
In this sense, the central zero acts as an \emph{additive singularity}, while the
ideal boundary plays the role of an effective multiplicative singularity.

\paragraph{Topological feature.}
Removing the singular point yields the punctured disk
$\mathbb{D}^2\setminus\{0\}$, whose first homology group is
\[
H_1(\mathbb{D}^2\setminus\{0\})\cong\mathbb{Z}.
\]
This is the first AEG example admitting non-contractible loops.
As a result, arithmetic histories encircling the singularity form distinct
homology classes, and the existence and cost of fillings become non-trivial.
This feature makes $\mathfrak{E}_1$ the natural testing ground for arithmetic
homotopy, filling cost, and the Stokes-type formulations developed later via the
Accumulative Commutative Space.
