...
$\mu,\lambda$.
We construct and analyse a concrete example, the \emph{zeroth kind space} $\mathfrak{E}_0$, realized on the upper
...
Within $\mathfrak{E}_0$ we identify addition–multiplication grids whose combinatorics reflect Baumslag–Solitar
...
\begin{itemize}
  ...
  zeroth kind space $\mathfrak{E}_0$ on the hyperbolic upper half–plane.
...
Section~\ref{sec:01kind} constructs the zeroth kind arithmetic expression space $\mathfrak{E}_0$ on the hyperbolic
upper half–plane, analyses its grid structures, and relates arithmetic torsion to hyperbolic area, and then gives an
outline for the first kind space $\mathfrak{E}_1$ which employs a singular point.
