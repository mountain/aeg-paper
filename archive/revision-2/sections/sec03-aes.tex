...
Fix constants $\mu,\lambda\in\mathbb{R}$. An \emph{Arithmetic Expression Space} (AES) is a tuple $(M,g,a,E_u,E_v)$ such that:
\begin{itemize}
    \item $M$ is an oriented two-dimensional Riemannian manifold with metric $g$;
    \item $a:M\to\mathbb{R}$ is a smooth scalar field;
    \item $M$ is equipped with an oriented orthonormal frame $(E_u,E_v)$ such that
    \begin{equation}\label{eq:aes-directional}
        E_u(a)=\mu,
        \qquad
        E_v(a)=\lambda a.
    \end{equation}
\end{itemize}
We call $(E_u,E_v)$ the \emph{propagation frame}.
\end{definition}

The manifold $M$ provides the geometric stage, the scalar field $a$ records arithmetic assignment, and the parameters $\mu,\lambda$ encode the two elementary propagation intensities. The additional data of a propagation frame records which orthogonal directions are interpreted as additive and multiplicative propagation, resolving an ambiguity not determined by the eikonal norm alone.

\subsection{Compatibility and the eikonal form}\label{subsec:aes-eikonal}
The differential law in Section~\ref{subsec:flow-derivation} is directional: it describes the evolution of $a$ along a curve whose tangent is resolved into additive and multiplicative directions. In an AES this resolution is provided by the propagation frame.

\begin{proposition}[Flow form and eikonal consequence]\label{prop:aes-flow-eikonal}
Let $(M,g,a,E_u,E_v)$ be an AES. For a unit-speed curve $\gamma(s)$ whose unit tangent has the decomposition
\[
\dot\gamma(s)=\cos\theta(s)\,E_u+\sin\theta(s)\,E_v,
\]
one has the arithmetic flow law
\begin{equation}\label{eq:aes-flow}
\frac{d}{ds}a(\gamma(s))=\mu\cos\theta(s)+\lambda a(\gamma(s))\sin\theta(s).
\end{equation}
Moreover, $a$ satisfies the eikonal relation
...
\begin{proof}
The flow law is immediate from \eqref{eq:aes-directional}:
\[
\frac{d}{ds}a(\gamma(s))=\langle \nabla a,\dot\gamma(s)\rangle_g=E_u(a)\cos\theta(s)+E_v(a)\sin\theta(s).
\]
For the eikonal relation, decompose $\nabla a$ in the orthonormal frame:
\[
\nabla a=E_u(a)\,E_u+E_v(a)\,E_v=\mu\,E_u+\lambda a\,E_v.
\]
Taking norms yields \eqref{eq:aes-eikonal}.
\end{proof}

Equation \eqref{eq:aes-eikonal} is an eikonal equation, equivalently a first-order Hamilton--Jacobi-type relation in the modern PDE sense \cite{Hamilton1834GeneralMethod,Evans2010PDE}.
...
\begin{equation}\label{eq:e0-frame}
E_u\coloneqq -\mu y\,\partial_x,
\qquad
E_v\coloneqq -\lambda y\,\partial_y.
\end{equation}

\begin{theorem}\label{thm:aes-e0}
The tuple $(\mathcal{H},g_{\mu,\lambda},a,E_u,E_v)$ is an Arithmetic Expression Space in the sense of Definition~\ref{def:aes}. We call it the \emph{zeroth kind arithmetic expression space} and denote it by $\mathfrak{E}_0=\mathfrak{E}_0(\mu,\lambda)$.
\end{theorem}

\begin{proof}
The fields \eqref{eq:e0-frame} are orthonormal and oriented for the metric \eqref{eq:e0-metric}. Moreover, $E_u(a)=\mu$ and $E_v(a)=\lambda a$, so \eqref{eq:aes-directional} holds.
We compute
\[
a_x=-\frac{1}{y},
\qquad
...
Away from $r=0$, this defines a smooth assignment field on the punctured disc $\mathbb{D}\setminus\{0\}$.

\begin{lemma}\label{lemma:aes-e1}
On $\mathbb{D}\setminus\{0\}$, the metric \eqref{eq:e1-metric} and the field \eqref{eq:e1-assignment} satisfy the AES eikonal relation \eqref{eq:aes-eikonal}.
\end{lemma}
...
At every point of $\mathbb{D}\setminus\{0\}$ one may choose an oriented orthonormal frame $(e_r,e_\vartheta)$ such that $e_\vartheta(a)=0$ and $e_r(a)=\|\nabla a\|$. Define an angle function $\psi$ by
\[
\cos\psi=\frac{\mu}{\sqrt{\mu^2+\lambda^2 a^2}},
\qquad
\sin\psi=\frac{\lambda a}{\sqrt{\mu^2+\lambda^2 a^2}}.
\]
Then the rotated frame
\[
E_u\coloneqq \cos\psi\,e_r-\sin\psi\,e_\vartheta,
\qquad
E_v\coloneqq \sin\psi\,e_r+\cos\psi\,e_\vartheta
\]
satisfies \eqref{eq:aes-directional}. With any such choice, $(\mathbb{D}\setminus\{0\},g_1,a,E_u,E_v)$ is an AES.
