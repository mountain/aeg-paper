...
\subsection{A calculable family: the affine--Appell basis}\label{subsec:analytic-affine-appell}
The operator package above becomes substantially more concrete once one identifies a stable family on which $\partial_{\mathrm{AEG}}$ and $\bar\partial_{\mathrm{AEG}}$ act in a controlled way.

Let
\[
q\coloneqq e^{\lambda v},
\qquad
B_n(a,v)\coloneqq q^{-n}a^n=e^{-n\lambda v}a^n
\qquad (n\in\mathbb{N}).
\]
...
\]
with each $P_n$ independent of $a$, the horizontal operators act triangularly with respect to the index $n$.

\begin{proposition}\label{prop:analytic-affine-appell-operators}
...
\frac{\mu n}{2q}\,P\,B_{n-1},
\]
with the convention $B_{-1}=0$. In particular, for each $N$ the finite span $\mathrm{span}\{B_0,\dots,B_N\}$ is stable under $\partial_{\mathrm{AEG}}$, $\bar\partial_{\mathrm{AEG}}$, and $\Delta_H$.
\end{proposition}

\begin{proof}
Since $P$ is independent of $a$, one has $D_uP=\partial_uP$ and $D_vP=\partial_vP+\lambda q\,\partial_qP$. Moreover, $D_u a=\mu$, $D_v a=\lambda a$, and $D_vq=\lambda q$, so
\[
D_uB_n=\mu n q^{-1}B_{n-1},
\qquad
D_vB_n=0.
\]
Therefore
...
\]
Substituting these into \eqref{eq:analytic-dbar} gives the stated formulas. Stability follows because $\Delta_H=D_u^2+D_v^2$ preserves the same finite span.
\end{proof}
