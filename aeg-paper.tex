...
\title{Geometry of Arithmetic Expressions: I.\\ Concepts and Unsolved Problems}
...
\section{Algebraic structure of generated threadlike arithmetic expressions}\label{sec:algebraicstructure}
...
Suppose we have a series of operators $a_1, a_2, \cdots a_{n-1}, a_n$, we introduce the initial version of \emph{path notation}.
...
In order to make this notation purely functional, we can also introduce a special operator $\odot$, and interpret it as
\begin{itemize}
    \item When it appears at the beginning of a path and is followed by the operator $\oplus_y$ or $\ominus_y$, it is interpreted as the zero map $\odot: x \mapsto 0$
    \item When it appears at the beginning of a path and is followed by the operator $\otimes_y$ or $\oslash_y$, it is interpreted as the one map $\odot: x \mapsto 1$
    \item When it does not appear at the beginning of a path, it is interpreted as the identity map $\odot: x \mapsto x$
\end{itemize}

That is the final version of our path notation. Now, we can rewrite the above example as follows:

$$\odot \oplus_1 \otimes_{\ln 2} \otimes_{\ln 2} \oplus_1 \otimes_{\ln 3} \oplus_6 $$

If a path begins with $\odot$, we refer to it as a \emph{bounded path}.
...
\begin{definition}\label{definition:gca}
    The greatest common ancestor of two bounded paths $\beta, \gamma$ is defined as the composite of functions:
    $$p_1 \cdot p_2 \coloneqq p_2 \circ p_1 $$
\end{definition}

\begin{figure}[ht]
\centering
\resizebox{0.5\textwidth}{!}{\includegraphics{images/10-gca.pdf}}
\caption{Factorization by greatest common ancestor}
\end{figure}

\subsection{Algebraic structure and equality}

For a given number field $\mathcal{F}$ and elements $\mu, \lambda \in \mathcal{F}$, we consider all the arithmetical expressions
$E(\mu, \lambda)$ that are freely generated from
\begin{itemize}
...
$\mu$ serves as an additional generator, and $e^\lambda$ is the multiplicative generator.
In cases where the context is clear, we may omit $\mu$ and $\lambda$ from the index.

The algebraic structure of generated arithmetic expressions will depend on the type of equality we adopt:
\begin{itemize}
    \item literal structure: a semigroup under the literal equality
    \item syntactical structure: under equality by some syntactical rules
    \begin{itemize}
        \item when $\odot$ operator take no effect, it is a monoid
        \item when inverse operator exists, it is a group
        \item when commutative and distributive laws exist between additional operators and multiplicative operators, it is an algebra
    \end{itemize}
    \item semantic structure: under the equality of the evaluation of the expression
\end{itemize}

\subsection{Literal structure}\label{subsec:literial}

When we adopt literal equality, two expressions are equal if and only if they have the same string representation.


Any expressions in $E(\mu, \lambda)$ can be written as a path with operand $0$.
...
We can introduce the inverse notation
\begin{itemize}
    \item $\oplus^{-1} = \ominus$
    \item $\ominus^{-1} = \oplus$
...
Comparing with path notation, we call this an expanded notation, or evaluated notation if we emphasising on evaluated value.
...
\begin{proof}
By using the expanded notation, it is easy to verify the three axioms of an algebra is hold:
...
\begin{itemize}
\item $\theta = 0$: $a_1 = a_0 + \mu s$
\item $\theta = \frac{\pi}{2}$: $a_1 = a_0 e^{\lambda s}$
\item $\theta = \pi$: $a_1 = a_0 - \mu s$
\item $\theta = \frac{3 \pi}{2}$: $a_1 = a_0 e^{- \lambda s} $
\end{itemize}
...
On the hyperbolic plane with sectional curvature $\kappa$, the circumference of the circle with radius $r$ is given by the following formula

\begin{equation}
c = \frac{\sinh(\sqrt{\kappa} r)}{\sqrt{\kappa}}\label{eq:circumference}
\end{equation}
...
\section{Unsolved problems}\label{sec:secondkind}
