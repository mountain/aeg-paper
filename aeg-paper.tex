...
\begin{equation}
   a = (a_0 + \frac{\mu}{\lambda} \cot \theta) e^{s \lambda \sin \theta} - \frac{\mu}{\lambda} \cot \theta
\end{equation}

\begin{equation}
   a =  a_0 e^{s \lambda \sin \theta} + \frac{\mu}{\lambda} (e^{s \lambda \sin \theta} - 1) \cot \theta
\end{equation}
...
and so on. And each time the cell of the grid mesh is divided into smaller cells and the end points of the vertical lines
move upward.
